package edu.rosehulman.gateslm.FlowAnalysisGrouping.SetReducer;
...
public class RegionFlowAnalysisGroupReducer extends Reducer<Text, RegionFlowAnalysisGroupingSet, Text, Text> {
	
	public final int TOTAL_SECONDS_IN_DAY = 60 * 60 * 24;

	@Override
	protected void reduce(Text key, Iterable<RegionFlowAnalysisGroupingSet> values,
		...
		for (RegionFlowAnalysisGroupingSet value : values) {
			totalDays++;
		...
		if (intDay > .05 && maxSIC.getSpeed() < 25) {
